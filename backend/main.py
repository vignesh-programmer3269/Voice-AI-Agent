...
@app.get("/api/doctors")
def api_get_doctors():
    return {"doctors": ["Dr. Sharma (Cardiologist)", "Dr. Iyer (Dermatologist)", "Dr. Singh (Neurologist)", "Dr. Patel (Pediatrics)"]}
...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    greeting_text = "Welcome to 2Care Clinic. How can I assist you with your appointments today?"
    ...
    try:
        while True:
            ...
            if not text.strip():
                ...
                fallback_msg = "I'm sorry, I didn't quite catch that. Could you please repeat?"
                if lang == "hi": fallback_msg = "माफ़ कीजिये, मैं समझ नहीं पाया। क्या आप दोहरा सकते हैं?"
                if lang == "ta": fallback_msg = "மன்னிக்கவும், எனக்கு புரியவில்லை. மீண்டும் சொல்ல முடியுமா?"
                
                audio_response = await generate_speech(fallback_msg, lang)
                await websocket.send_text(json.dumps({
                    "type": "metrics", "stt": round(stt_latency), "llm": 0, "tts": 100, "total": round(stt_latency + 100)
                }))
                await websocket.send_bytes(audio_response)
                continue
            ...
            llm_text, updated_history = get_agent_response(text, history, lang, patient_name)
